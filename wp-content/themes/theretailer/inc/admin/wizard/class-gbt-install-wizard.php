...
if ( ! class_exists( 'GBT_InstallWizard' ) ) {
	...
	class GBT_InstallWizard {
		...
		public function setupWizard() {
			...
			$default_steps = array(
				'introduction' => array(
					'name'    => esc_html__( 'Introduction', 'theretailer' ),
					'view'    => array( $this, 'gbt_setup_introduction' ),
					'handler' => '',
				),
				'plugins' => array(
					'name'    => esc_html__( 'Plugins', 'theretailer' ),
					'view'    => array( $this, 'gbt_setup_plugins' ),
					'handler' => '',
				),
				'demo' => array(
					'name'    => esc_html__( 'Demo Import', 'theretailer' ),
					'view'    => array( $this, 'gbt_setup_demo' ),
					'handler' => '',
				),
				'final' => array(
					'name'    => esc_html__( 'Ready!', 'theretailer' ),
			...
			wp_enqueue_style( 'gbt-wizard-css', get_template_directory_uri() .'/inc/admin/wizard/css/wizard.css', array(), getbowtied_theme_version() );
			wp_register_script( 'gbt-wizard-js', get_template_directory_uri() .'/inc/admin/wizard/js/wizard.js', array( 'jquery' ), getbowtied_theme_version() );
		...
		public function setup_wizard_steps() {
			...
			<div class="gbt-wizard-logo <?php echo getbowtied_theme_slug(); ?>">
				<a href="#">
					<img src="<?php echo get_template_directory_uri() .'/inc/admin/wizard/images/theretailer-logo.png'; ?>" alt="Logo">
				</a>
			</div>
		...
		public function setup_wizard_footer() {
			?>
					...
					<a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><span class="dashicons dashicons-arrow-left-alt"></span><?php esc_html_e( 'Return to the WordPress Dashboard', 'theretailer' ); ?></a>
		...
		public function gbt_setup_introduction() {
			?>
				<div class="wrapper wizard-introduction" style="background-image:url(<?php echo get_template_directory_uri() .'/inc/admin/wizard/images/theretailer-setup.jpg'; ?>);">
					<div class="center">
						<h1><?php esc_html_e( getbowtied_theme_name() . '\'s', 'theretailer');?> <br/> <?php esc_html_e('Theme Setup Wizard', 'theretailer' ); ?></h1>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							class="button-primary button button-large button-next">
							<?php esc_html_e( 'Get Started', 'theretailer' ); ?>
						</a>
						<p><?php esc_html_e( 'The quickest way to setup the theme', 'theretailer'); ?> <br /> <?php esc_html_e('and start working on your site.', 'theretailer' );?></p>
					</div>
				</div>
			...
			?>
				<div class="wrapper wizard-plugins">
					...
					<div class="content-info">
						<h1><?php esc_html_e( 'Plugin Installation', 'theretailer' ); ?></h1>
						<p><?php esc_html_e( 'Install the required plugins before importing the demo content.', 'theretailer' ); ?></p>
					</div>
					...
					<ul class="plugins">
						...
						<li class="recommended">
							<h3><?php esc_html_e( 'Recommended', 'theretailer' ); ?></h3>
						</li>
						<?php $this->_get_plugins_list($plugins, false); // recommended plugins ?>
					</ul>

					<div class="buttons">
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button skip"><?php esc_html_e( 'Skip', 'theretailer' ); ?></a>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button install "><?php esc_html_e( 'Install', 'theretailer' ); ?></a>
					</div>
				</div>
					...
					<li class="plugin <?php echo esc_attr( $slug ); ?>">
						...
						<label for="<?php echo esc_attr($plugin['slug']); ?>" class="<?php echo ( isset($plugin['demo_required']) && $plugin['demo_required'] === true ) ? 'required' : 'optional' ?>">
							...
							<div class="plugin-description-container">
								...
								<p><?php echo isset( $plugin['description'] )? esc_html_e( $plugin['description'], 'theretailer' ) : ''; ?></p>
							</div>
							...
							<div class="plugin-install">
								...
								<?php if ( ! empty( $plugin['status'] ) && ($plugin['status'] != 'no-action') ) : ?>
										<a  class="button ajax-request <?php echo esc_html( $plugin['status'] ); ?>-now"
											...
											<?php echo ( isset($plugin['status']) && $plugin['status'] == 'install' ) ? esc_html_e( 'Install Now', 'theretailer' ) : ''; ?>
											<?php echo ( isset($plugin['status']) && $plugin['status'] == 'update' ) ? esc_html_e( 'Update Now', 'theretailer' ) : ''; ?>
											<?php echo ( isset($plugin['status']) && $plugin['status'] == 'activate' ) ? esc_html_e( 'Activate', 'theretailer' ) : ''; ?>
										</a>

									<?php else : ?>
										<a class="button button-disabled">
											<?php  esc_html_e( 'Active', 'theretailer' ); ?>
										</a>
									<?php endif; ?>
								</div>
							</div>
		...
		public function gbt_setup_demo() {
			...
			<div class="wrapper wizard-demo-import">
				...
				<?php $is_required_plugins = ( TR_WOOCOMMERCE_IS_ACTIVE && TR_PRODUCT_BLOCKS_IS_ACTIVE && class_exists( 'OCDI_Plugin' ) ) ? true : false; ?>

				<div class="content-info">
					<h1><?php esc_html_e( 'Demo Content Import', 'theretailer' ); ?></h1>
					<p><?php esc_html_e( 'Start with pre-built page layouts, dummy product pages,', 'theretailer');?><br/> <?php esc_html_e('blog posts and widgets.', 'theretailer' ); ?></p>
				</div>

				<div class="demo-icon <?php echo ( ! $is_required_plugins ) ? 'error' : '' ?>">
					...
					<?php else : ?>
						<p class="error-info"><?php esc_html_e( 'Please make sure WooCommerce and Product Blocks for WooCommerce are installed and activated before importing the demo content.', 'theretailer' ); ?></p>
					<?php endif; ?>
				</div>
				...
				<div class="buttons">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button skip"><?php esc_html_e( 'Skip', 'theretailer' ); ?></a>
					<?php if ( $is_required_plugins ) : ?>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button install"><?php esc_html_e( 'Continue', 'theretailer' ); ?></a>
					<?php else : ?>
						<a href="<?php echo esc_url( $this->get_prev_step_link() ); ?>" class="button "><?php esc_html_e( 'Go back', 'theretailer' ); ?></a>
					<?php endif; ?>
				</div>
		...
		public function gbt_setup_final() {
			...
			<div class="wrapper wizard-ready">
				<div class="content-done" style="background-image:url(<?php echo get_template_directory_uri() .'/inc/admin/wizard/images/theretailer-setup.jpg'; ?>);">
					<div class="center">
						<h1><?php esc_html_e( 'Setup has been completed successfully!', 'theretailer' ); ?></h1>
						<a href="<?php echo esc_url( site_url() ); ?>" class="button button-primary"><?php esc_html_e( 'View Site', 'theretailer' );?></a>
						<p><?php esc_html_e('You should be able to start working on your site now.','theretailer');?> <br/>
						<?php esc_html_e('Best of luck with your project!', 'theretailer'); ?></p>
					</div>
				</div>

				<div class="further-info">
					<h2><?php esc_html_e( 'What\'s next?', 'theretailer' );?></h2>
					<div class="column">
						...
						<ul>
							<li>
								<a href="<?php echo esc_url( admin_url( 'customize.php' ) ); ?>">
									...
									<?php esc_html_e( 'Customize the theme', 'theretailer' ); ?>
								</a>
							</li>
							<li>
								<a href="<?php echo esc_url( admin_url( 'index.php?page=wc-setup' ) );?>">
									<span class="dashicons dashicons-admin-settings"></span>
									<?php esc_html_e( 'WooCommerce Setup Wizard', 'theretailer' ); ?>
								</a>
							</li>
						</ul>
					</div>
						...
						<ul>
							<li>
								...
								<a href="<?php print esc_url( $activate ); ?>">
									<span class="dashicons dashicons-update"></span><?php esc_html_e('Activate Theme Updates', 'theretailer' ); ?>
								</a>
							</li>
							<li><a href="https://www.getbowtied.com/docs/the-retailer/" target="_blank"><span class="dashicons dashicons-editor-help"></span><?php esc_html_e( 'Help Center / Support', 'theretailer' ); ?></a></li>
						</ul>
					</div>
				</div>
		...
		public function _get_plugins() {
			...
			$newplugins['the-retailer-extender'] = $plugins['the-retailer-extender'];
			...
			$newplugins['woocommerce-colororimage-variation-select'] = $plugins['woocommerce-colororimage-variation-select'];
